...
$$
\boxed{\text{MA}_{j^{\ast}} = \sum_{i=1}^{K} \sigma_i^{(L)} \cdot \bigl(h_2^{(L)} \cdot v_i^{(L)}\bigr) \cdot u_i^{(L)}[j^{\ast}] + b^{(L)}[j^{\ast}]}
$$
...
$$
\text{MA}_{j^{\ast}} = \sum_{L \in \mathcal{L}_{\text{origin}}} \left[ \sum_{i=1}^{K} \sigma_i^{(L)} \cdot \bigl(h_2^{(L)} \cdot v_i^{(L)}\bigr) \cdot u_i^{(L)}[j^{\ast}] + b^{(L)}[j^{\ast}] \right]
$$
...
$$
\boxed{\text{MA}_{j^{\ast}} \approx \sigma_1^{\text{macro}} \cdot \bigl(h_2 \cdot v_1^{\text{macro}}\bigr) \cdot u_1^{\text{macro}}[j^{\ast}] + \text{higher-order}}
$$
...
公式各项 $\sigma_i \cdot (h_2 \cdot v_i) \cdot u_i[j^{\ast}]$ 对 MA 的贡献 **符号一致同向叠加**：

$$
\text{cos}(h_2, v_i) \approx 0 \text{（弱对齐）但} \quad \text{sign}\bigl[\sigma_i (h_2 \cdot v_i) u_i[j^{\ast}]\bigr] \text{ 跨 } i \text{ 一致}
$$
...
| **D2：j\* 共享** | 不同 $i$ 的 $u_i[j^{\ast}]$ 都集中在同一稀疏维度 | **稀疏 readout** —— MA 永远落在同 1-2 个 hidden 维度 |
...
\boxed{
\begin{aligned}
&\textbf{方向条件 (sign 一致)}: & \quad &\text{sign}(h_2 \cdot v_i) \cdot \text{sign}(u_i[j^{\ast}]) \text{ 跨 } i \text{ 同号} \\[2pt]
&\textbf{稀疏条件 (j\* 集中)}: & \quad &|u_i[j^{\ast}]| \gg |u_i[j']| \quad \forall j' \neq j^{\ast} \\[2pt]
