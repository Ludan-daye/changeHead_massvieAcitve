...
def run_experiment(args, mode='baseline', enable_heads_dict=None):
    ...
    if mode != 'baseline':
        ...
        for layer_id in range(len(layers)):
            ...
            num_heads = getattr(model.config, 'n_head', None) or getattr(model.config, 'num_attention_heads', None) or getattr(model.config, 'n_heads', None)
            if num_heads is None:
                raise ValueError(f"Cannot determine num_heads for model {args.model}")
    ...
    # Detect MA dimensions dynamically after first forward pass
    ma_dims = None  # Will be set after first sample
    ...
    for layer_id in range(n_layers):
        layer_stats[layer_id] = {
            ...
            'top3': [],
            'median': [],
            'ma_dim0': [],  # Track top MA dimension (dynamically detected)
            'ma_dim1': []   # Track second MA dimension
        }
    ...
    with torch.no_grad():
        for idx, testseq in enumerate(tqdm(testseq_list, desc=f"Processing ({mode})")):
            ...
            for layer_id in range(n_layers):
                ...
                # Detect MA dimensions dynamically from first sample
    ...
    for layer_id in range(n_layers):
        ...
        if layer_id in key_layers:
            ...
            print(f"  MA Dim 0: {results[layer_id]['ma_dim0_mean']:.2f} (max: {results[layer_id]['ma_dim0_max']:.2f})")
            print(f"  MA Dim 1: {results[layer_id]['ma_dim1_mean']:.2f} (max: {results[layer_id]['ma_dim1_max']:.2f})")
...
def generate_visualizations(baseline_results, disabled_results, baseline_stats, disabled_stats, savedir):
    ...
    ax.plot(layers, disabled_ma1, 's-', linewidth=3, markersize=8, label='All Heads Disabled', color='#A23B72')
    ax.set_xlabel('Layer', fontsize=12, fontweight='bold')
    ...
    ax = axes[1, 1]
    baseline_138 = [baseline_results[i]['ma_dim0_mean'] for i in layers]
    disabled_138 = [disabled_results[i]['ma_dim0_mean'] for i in layers]
    ...
    ax.fill_between(layers, disabled_ma1_mean, disabled_ma1_max, alpha=0.2, color='#A23B72')
    ...
    ax = axes[1]
    baseline_138_mean = [baseline_results[i]['ma_dim0_mean'] for i in layers]
    disabled_138_mean = [disabled_results[i]['ma_dim0_mean'] for i in layers]
    baseline_138_max = [baseline_results[i]['ma_dim0_max'] for i in layers]
    disabled_138_max = [disabled_results[i]['ma_dim0_max'] for i in layers]
...
def generate_summary_report(baseline_results, disabled_results, savedir):
    ...
    ma_dim1_baseline_peak = max([baseline_results[i]['ma_dim1_mean'] for i in range(n_layers)])
    ma_dim1_disabled_peak = max([disabled_results[i]['ma_dim1_mean'] for i in range(n_layers)])
    ma_dim1_peak_layer = max(range(n_layers), key=lambda i: baseline_results[i]['ma_dim1_mean'])
    ma_dim1_change_pct = 100 * (ma_dim1_disabled_peak - ma_dim1_baseline_peak) / ma_dim1_baseline_peak if ma_dim1_baseline_peak > 0 else 0

    # Dimension 138 (secondary)
    ma_dim0_baseline_peak = max([baseline_results[i]['ma_dim0_mean'] for i in range(n_layers)])
    ma_dim0_disabled_peak = max([disabled_results[i]['ma_dim0_mean'] for i in range(n_layers)])
    ma_dim0_peak_layer = max(range(n_layers), key=lambda i: baseline_results[i]['ma_dim0_mean'])
    ma_dim0_change_pct = 100 * (ma_dim0_disabled_peak - ma_dim0_baseline_peak) / ma_dim0_baseline_peak if ma_dim0_baseline_peak > 0 else 0
    ...
    report_lines.append(f"  Baseline Peak Value:       {ma_dim1_baseline_peak:.2f} (Layer {ma_dim1_peak_layer})")
    report_lines.append(f"  All Heads Disabled Value:  {ma_dim1_disabled_peak:.2f}")
    report_lines.append(f"  Absolute Change:           {ma_dim1_disabled_peak - ma_dim1_baseline_peak:.2f}")
    report_lines.append(f"  Percentage Change:         {ma_dim1_change_pct:.2f}%")
    report_lines.append("")

    if abs(ma_dim1_change_pct) < 5:
        conclusion_ma = "✅ MINIMAL IMPACT - Massive activations persist without heads"
    elif ma_dim1_change_pct < -20:
        conclusion_ma = "⚠️ SIGNIFICANT DECREASE - Heads may contribute to generation"
    else:
        conclusion_ma = "🔍 MODERATE CHANGE - Partial contribution from heads"
    ...
    report_lines.append("-" * 70)
    report_lines.append(f"  Baseline Peak Value:       {ma_dim0_baseline_peak:.2f} (Layer {ma_dim0_peak_layer})")
    report_lines.append(f"  All Heads Disabled Value:  {ma_dim0_disabled_peak:.2f}")
    report_lines.append(f"  Absolute Change:           {ma_dim0_disabled_peak - ma_dim0_baseline_peak:.2f}")
    report_lines.append(f"  Percentage Change:         {ma_dim0_change_pct:.2f}%")
    report_lines.append("")

    if abs(ma_dim0_change_pct) < 5:
        conclusion_138 = "✅ MINIMAL IMPACT - Massive activations persist without heads"
    elif ma_dim0_change_pct < -20:
...
def main():
    ...
    # Compute model-level summary for Table 1
    n_layers = len(baseline_results)
    key_layer = max(range(n_layers), key=lambda i: baseline_results[i]['top1_mean'])
    base_ma = baseline_results[key_layer]['top1_mean']
    disabled_top1 = disabled_results[key_layer]['top1_mean']
    delta_top1 = (disabled_top1 - base_ma) / base_ma * 100 if base_ma > 0 else 0

    table1_summary = {
        'model': args.model,
        'key_layer': key_layer,
        'base_ma': float(base_ma),
        'delta_top1_pct': float(delta_top1),
    }
    with open(os.path.join(args.savedir, 'table1_rq1.json'), 'w') as f:
        json.dump(table1_summary, f, indent=2)
    print(f"\n  Table 1 data: Key Layer={key_layer}, Base MA={base_ma:.2f}, ΔTop1={delta_top1:+.1f}%")
