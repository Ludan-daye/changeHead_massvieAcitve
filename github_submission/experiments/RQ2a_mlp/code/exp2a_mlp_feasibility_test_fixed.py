...
class MLPDisableHook:
    ...
    def __call__(self, module, input, output):
        ...
        Bug B7 fix (2026-04-21): handle tuple output correctly.
        MoE SparseMoeBlock returns (hidden_states, router_logits) — previously
        `torch.zeros_like(tuple)` had undefined behavior, causing partial zeroing
        (e.g. qwen3.5_35b_a3b retain=81% vs qwen3_30b_a3b retain=2.85%).
        Parallels exp6_progressive_ablation.py:32-35.
        """
        if self.mode == 'disable_all':
            # Handle tuple output (MoE): zero hidden, preserve router_logits
            if isinstance(output, tuple):
                return (torch.zeros_like(output[0]),) + output[1:]
            # Dense: simple tensor zero
