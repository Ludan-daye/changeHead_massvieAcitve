...
def get_mlp_weights(layer):
    ...
    if hasattr(layer.mlp, 'c_proj'):
        ...
        return W.T
    raise ValueError("Cannot find c_proj")

# GPT-2: 只有12层，选择代表性层（根据README，MA峰值在深层）
layer_ids = [0, 1, 2, 5, 8, 9, 10, 11]

print("="*80)
print("EXPERIMENT 4: GPT-2 MLP SVD Analysis")
...
# 保存结果
savedir = os.path.join(PROJECT_ROOT, 'results/experiments/exp4/gpt2')
...
print(f"\n{'='*80}")
print(f"✅ GPT-2完成！结果已保存至: {output_path}")
print(f"{'='*80}")
