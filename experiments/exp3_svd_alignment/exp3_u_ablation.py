"""
Experiment 3: U矩阵消融实验
测试MLP down_proj左奇异向量(U)对Massive Activation的影响
...
def ablate_u_remove_top_k(U, S, Vh, k):
    """方式A: 移除前k个U方向 (使用预计算的SVD)"""
    S_ablated = S.copy()
    S_ablated[:k] = 0
...
def ablate_u_keep_top_k(U, S, Vh, k):
    """方式B: 只保留前k个U方向 (使用预计算的SVD)"""
...
def run_ablation_experiment(args):
    ...
    for k in args.k_values:
        ...
        # 消融权重 (使用预计算的SVD)
        W_ablated, info = ablate_u_remove_top_k(U, S, Vh, k)
    ...
    for k in args.k_values:
        ...
        # 消融权重 (使用预计算的SVD)
        W_ablated, info = ablate_u_keep_top_k(U, S, Vh, k)
...
def plot_results(results, savedir):
    ...
    ax2.set_title(f'方式B: 只保留前k个U方向\n(Baseline={baseline:.1f})')
    ax2.axhline(y=0, color='black', linestyle='--', alpha=0.5)
    
    plt.suptitle(f'{results["model"]} - Layer {results["critical_layer"]} U矩阵消融实验', 
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    
    plt.savefig(os.path.join(savedir, 'u_ablation_results.png'), dpi=150, bbox_inches='tight')
    plt.close()
    print(f"图表已保存: {os.path.join(savedir, 'u_ablation_results.png')}")
...
def main():
    ...
    with open(os.path.join(args.savedir, 'u_ablation_results.json'), 'w') as f:
        json.dump(results, f, indent=2)
    print(f"\n结果已保存: {os.path.join(args.savedir, 'u_ablation_results.json')}")
