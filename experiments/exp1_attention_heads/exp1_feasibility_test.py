...
def run_experiment(args, mode='baseline', enable_heads_dict=None):
    ...
    for layer_id in range(len(layers)):
        if is_llama_model(args.model) or "deepseek" in args.model or "mistral" in args.model:
            mp.enable_llama_custom_decoderlayer(layers[layer_id], layer_id)
        elif "qwen" in args.model:
            mp.enable_qwen_custom_decoderlayer(layers[layer_id], layer_id)
            ...
            gptj_mp.enable_gptj_custom_block(layers[layer_id], layer_id)
        elif "bloom" in args.model:
            mp.enable_bloom_custom_block(layers[layer_id], layer_id)
        elif "falcon" in args.model:
            falcon_mp.enable_falcon_custom_decoderlayer(layers[layer_id], layer_id)
        else:
            raise ValueError(f"Model {args.model} not supported for feature capture in exp1.")
    ...
    if mode != 'baseline':
        ...
        for layer_id in range(len(layers)):
            ...
            if is_llama_model(args.model) or "opt" in args.model or "qwen" in args.model or "deepseek" in args.model or "mistral" in args.model:
                num_heads = model.config.num_attention_heads
                target_module = layer.self_attn
            elif "gpt2" in args.model or "gptj" in args.model:
                num_heads = model.config.n_head
                target_module = layer.attn
            elif "bloom" in args.model:
                num_heads = model.config.n_head
                target_module = layer.self_attention
            elif "falcon" in args.model:
                num_heads = model.config.num_attention_heads
                target_module = layer.self_attention
            else:
                raise ValueError(f"Model {args.model} not supported for head disabling in exp1.")
