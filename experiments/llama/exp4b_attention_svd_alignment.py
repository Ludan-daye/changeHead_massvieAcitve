...
Targeted Research:
- Focus on Layer 3 (starting layer of massive activations)
- Analyze attention head weight matrices (Q, K, V, O projection)
- Perform alignment analysis between right singular vectors of these matrices and activation directions

Research Question:
Do the right singular vectors of Layer 3 attention head weight matrices align with
the massive activation directions generated by this layer?
"""
...
def compute_attention_svd(layer, layer_id, model_config):
    """
    Perform SVD decomposition on attention layer weight matrices
    Returns: SVD results for each weight matrix
    """
    print(f"\n{'='*80}")
...
def collect_layer3_activations(args, target_layer=3):
    """
    Collect Layer 3 activation values and their directions
    """
    print("\n" + "="*80)
    ...
    # Storage
    activation_data = {
        'input_vectors': [],      # Input vectors to Layer 3
        'output_vectors': [],     # Layer 3 output vectors
        'max_activations': [],    # Maximum activation values
        'max_positions': [],      # Positions of maximum activations
        'max_output_vectors': []  # Output vectors that produce maximum activations
    }
...
def analyze_attention_alignment(activation_data, svd_results, target_layer=3):
    """
    Analyze alignment degree between attention weight SVD directions and activation directions
    """
    print("\n" + "="*80)
    print(f"ANALYZING ATTENTION SVD ALIGNMENT FOR LAYER {target_layer}")
    print("="*80)
    
    # Get output activation vectors
    ...
    for weight_name, svd_data in svd_results.items():
        ...
        # Calculate cosine similarity between top 10 right singular vectors and mean output direction
        k = min(20, Vh.shape[0])
        alignments = []
