...
\section{Problem Formulation and Experimental Setup}
...
To systematically investigate the causes of massive activations, we adopt a multi-stage experimental framework, as outlined in Fig.~\ref{fig:Experimental_Framework}, which connects our hypothesis testing with causal verification. We begin by formally defining the key concepts and evaluation metrics, followed by implementation details. Each subsequent section then presents the methodology and results for one research question in an integrated manner.

\subsection{Metrics and Definitions}

Let $\mathbf{A} \in \mathbb{R}^{B \times L \times D}$ denote an internal activation tensor, where $B$, $L$, and $D$ represent batch size, sequence length, and hidden dimension, respectively.
...
\subsection{Models, Data, and Statistical Methods}

\textbf{Models:} All experiments are conducted across eight representative LLMs: GPT-2 (124M), LLaMA-2-13B, LLaMA-2-7B-Chat, BLOOM-7B1, GPT-J-6B, QWEN2.5-7B, OPT-6.7B, Falcon-7B, and Mistral-7B-v03, loaded from the Hugging Face Transformers library using standard configurations.

\textbf{Data:} We sample 64 documents independently from the C4 validation split (English, \texttt{allenai/c4}) using a fixed random seed (42), retaining only documents whose tokenized length is at least 256 BPE tokens. From each selected document we extract a single contiguous window of the first 256 tokens (no concatenation across documents), yielding 64 statistically independent observations per model. This protocol replaces the RedPajama-1T-Sample workflow used in the initial submission---both because that dataset is no longer hosted on the Hugging Face Hub and to ensure that every analysis window comes from a distinct source document, directly addressing the sample-dependency concerns raised in review.

To verify that this change does not affect the central conclusions, we performed two robustness checks under the new protocol across seven models (LLaMA-2-13B is excluded due to a tokenizer caching issue). First, we re-ran the function-word trigger analysis (RQ3): across all MA layers (top-1/median ratio $> 30\times$), function words account for 40.6--53.1\% of top-1 activation tokens (GPT-2: 53.1\%, Qwen2.5: 40.6\%, GPT-J: 51.6\%, Mistral: 41.0\%, BLOOM: 45.3\%, Falcon: 51.6\%), substantially exceeding the ${\sim}30\%$ base rate of function words in natural text. Second, we re-ran the $V$-matrix ablation (RQ5) on models whose key layer matches Table~\ref{tab:comprehensive_experimental_results}: GPT-2 ($\Delta\text{MA} = -75.9\%$, paper: $-70.7\%$, $\delta = 5.2$\,pp), Qwen2.5-7B ($-99.1\%$, $\delta = 0.0$\,pp), and GPT-J-6B ($-68.4\%$, paper: $-69.6\%$, $\delta = 1.2$\,pp). Both checks confirm that the core findings---function words preferentially trigger MAs, and $V$-matrix geometry is causally necessary---are robust to independent document-level sampling.

\textbf{Statistical Analysis:} For comparative experiments (RQ1, RQ3, RQ5), we employ paired t-tests with significance threshold $p < 0.01$. Effect sizes are reported using Cohen's $d$ where applicable. Correlation analyses use standard correlation coefficients with confidence intervals.


\section{Experiments and Results}

This section presents each research question with its methodology and results integrated, progressing from source identification (RQ1--RQ2) through trigger analysis (RQ3) to geometric mechanism discovery (RQ4) and causal verification (RQ5).

\subsection{(RQ1) Attention Mechanism Contribution Analysis}
\label{sec:RQ1}

\noindent\textbf{Hypothesis and Method.}
We test whether attention mechanisms directly generate MAs or merely modulate input patterns to downstream modules. For each model, we perform two inference passes: (1) \textit{Baseline}: normal forward pass; (2) \textit{Intervention}: all attention head outputs are dynamically zeroed via PyTorch hooks while preserving other components. We compare $\Delta$Top1 between conditions. A negative $\Delta$Top1 indicates attention promotes MA generation, while a positive value suggests suppression.
...
\noindent\textbf{Hypothesis and Method.}
Building on RQ1, this experiment directly compares activation magnitudes from attention and MLP sub-layers to identify the physical source of MAs. We hypothesize that MAs originate from MLP transformations rather than attention computations. At identified key layers, we simultaneously capture outputs from both attention ($\mathbf{H}_{\text{attn}}$) and MLP ($\mathbf{H}_{\text{mlp}}$) sub-layers. The ratio $\max(|\mathbf{H}_{\text{mlp}}|)/\max(|\mathbf{H}_{\text{attn}}|)$ quantifies the MLP's relative contribution. Values significantly greater than 1 confirm MLP as the primary MA source.
...
\noindent\textbf{Hypothesis and Method.}
This experiment investigates the linguistic correlates of MA triggering. We hypothesize that MAs are preferentially triggered by function words rather than content words. We record token positions corresponding to Top-K MAs and perform automated part-of-speech tagging using spaCy. The function word trigger rate is computed as:
\begin{equation}
...
\noindent\textbf{Hypothesis and Method.}
Given the MLP's identification as the MA source, this experiment examines the geometric structure of MLP weight matrices. We hypothesize that MA generation results from input alignment with dominant singular directions. For key MLP layers, we extract the down-projection matrix $\mathbf{W}$ and compute its SVD: $\mathbf{W} = \mathbf{U}\boldsymbol{\Sigma}\mathbf{V}^{\mathsf{T}}$. We analyze the singular value spectrum, focusing on the dominance ratio $\sigma_1/\sigma_2$. The MA direction vector $\mathbf{a}$ is compared with the top-$k$ right singular vectors via cosine similarity (Equation~4).
...
\subsection{(RQ5) V-Matrix Causal Validation}
\label{sec:RQ5}

\noindent\textbf{Hypothesis and Method.}
Building on the preceding experiments which established that massive activations originate from MLP layers and are triggered by function words through specific geometric alignments, we now investigate the causal role of the right singular matrix $V$ in this mechanism. To test whether $V$ matrix geometry is the core component determining MA generation, we designed a controlled ablation experiment across all baseline models. For a target MLP layer, we construct a modified weight matrix:
\begin{equation}
\mathbf{W}_{\text{ablated}} = \mathbf{U}\boldsymbol{\Sigma}\mathbf{V}_{\text{rand}}^{\mathsf{T}},
\end{equation}
where $\mathbf{V}_{\text{rand}}$ is a random orthogonal matrix with the same dimensions as $\mathbf{V}$, generated via QR decomposition of a Gaussian matrix. The original matrix $\mathbf{W}$ is replaced with $\mathbf{W}_{\text{ablated}}$ while keeping all other model parameters unchanged. This intervention isolates the geometric contribution of $V$ from the spectral amplification capacity encoded in $\boldsymbol{\Sigma}$, ensuring that any observed changes are attributable specifically to alterations in the input-output mapping directions defined by $V$.

\smallskip
\noindent\textbf{Results.}

\smallskip
\noindent\textbf{a. Systematic weakening of massive activations.}
